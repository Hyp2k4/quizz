...
export function NotificationInbox() {
    ...
    const getIcon = (type: string) => {
        switch (type) {
            ...
            default: return <Bell className="h-4 w-4 text-sky-500" />;
        }
    };
    ...
    return (
        <div className="relative">
            <Button 
                ...
                className={`rounded-full relative ${isOpen ? 'bg-sky-50 dark:bg-sky-950/20' : ''}`}
            >
            ...
            <AnimatePresence>
                {isOpen && (
                    <>
                        ...
                        >
                            {/* Header */}
                            <div className="p-4 border-b dark:border-zinc-800 bg-sky-50/50 dark:bg-sky-950/10 flex justify-between items-center">
                                    ...
                                    <div className="divide-y dark:divide-zinc-800">
                                        {notifications.map((notif) => (
                                            <div 
                                                ...
                                                className={`p-4 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50 relative group ${!notif.read ? 'bg-sky-50/20 dark:bg-sky-900/5' : ''}`}
                                            >
                                                {!notif.read && (
                                                ...
                                                <div className="flex gap-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className={`text-sm font-bold truncate ${!notif.read ? 'text-sky-600' : ''}`}>{notif.title}</p>
                            ...
                            {notifications.length > 0 && (
                                <div className="p-3 border-t dark:border-zinc-800 text-center">
                                    <button 
                                        ...
                                        className="text-[10px] font-bold text-zinc-400 hover:text-sky-500 uppercase tracking-widest"
                                    >
                                        Làm mới thông báo
                                    </button>
                                </div>
